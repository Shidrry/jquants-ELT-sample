...
from libs.utils_jquants import fetch_jquants_bars_daily

# ---- 新しいパイプライン作成時にここだけ変更する -------------------------
GCS_OBJECT = "ohlcv/dt={date}/daily_quotes.parquet"
BUCKET_FN  = jquants_lake_bucket  # (project_id, env_name) -> str

# v2 API の短縮カラム名を v1 相当の正式名に正規化する。
# BigQuery のスキーマを変えずに v2 へ移行できる。
# Date / Code は v1 と同名のためマッピング不要。
_V2_TO_V1_COLUMNS = {
    "O":      "Open",
    "H":      "High",
    "L":      "Low",
    ...
    "MAdjO":  "MorningAdjustmentOpen",
    "MAdjH":  "MorningAdjustmentHigh",
    "MAdjL":  "MorningAdjustmentLow",
    "MAdjC":  "MorningAdjustmentClose",
    "MAdjVo": "MorningAdjustmentVolume",
    # 後場（Premium）
    "AO":     "AfternoonOpen",
    "AH":     "AfternoonHigh",
    "AL":     "AfternoonLow",
    ...
    "AAdjO":  "AfternoonAdjustmentOpen",
    "AAdjH":  "AfternoonAdjustmentHigh",
    "AAdjL":  "AfternoonAdjustmentLow",
    "AAdjC":  "AfternoonAdjustmentClose",
    "AAdjVo": "AfternoonAdjustmentVolume",
}

def fetch_records(project_id: str, date_str: str) -> list[dict]:
    api_key = access_secret("jquants-api-key", project_id)
    return list(fetch_jquants_bars_daily(date_str, api_key))
# -------------------------------------------------------------------------
...
def main() -> None:
    ...
    df = pd.DataFrame(records).rename(columns=_V2_TO_V1_COLUMNS)
    upload_bytes_to_gcs(bucket, object_path, dataframe_to_parquet_bytes(df))

    logging.info("Fetched daily quotes %s", {
